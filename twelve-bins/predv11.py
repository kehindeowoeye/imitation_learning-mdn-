...
dataset = np.array(pd.read_excel('vv11.xlsx',header=None))
...
one_hot_label = np.array(pd.get_dummies(np.array(labeltrain.astype(int).reshape(-1))))


for i in range(nb_samples):
    y_position = i + look_back
    x_train_reshaped2[i] = Xtrain[i:y_position]
    one_hot_labels2[i] = one_hot_label[y_position,:2]
...
model.add(Bidirectional(LSTM(n_neurons,return_sequences=True,activation = 'relu'), input_shape=(None, num_features)))
...
history = model.fit(x_train_reshaped2, one_hot_labels2, epochs=n_epoch, batch_size=n_batch, verbose=2,validation_split = 0.3,callbacks=callbacks_list)
